...
function ptimetable(this_year, month)
{
	/* this table shows prayer time from dec 2006 to jan 2020*/
	var year_index = this_year - 2006;
	...
	/*  If this_year is not a leap year, date_index needs adjustment */
	if (is_leapyear(this_year) == 0)
		if (month_index > 1) 
			date_index--;
	...
	{
		if ((month + 1) < 10) {
			printMonth = "0" + (month + 1).toString();
		} else {
			printMonth = (month + 1).toString();
		}

		if ((i+1) < 10) {
			printDay = "0" + (i + 1).toString();
		} else {
			printDay = (i + 1).toString();
		}
	
		j_date =  printMonth + "/" + printDay + "/" + this_year.toString();
		
		newData = {};
		newData.date = j_date;

		var adjust_time = day_light_saving(month, i, k);
		newData.dayName = day[k];
		newData.beginning = {};
		newData.eqama = {};
		...
		for (j = 0; j < 11; j++) /* 2nd for loop, 11 prayer times of a day */
		{
			switch(j) {
				case 0:
					newData.beginning.fajr = set_time(month, i, j, adjust_time);
					break;
				case 1:
					newData.beginning.shorok = set_time(month, i, j, adjust_time);
					break;
				case 2:
					newData.beginning.dhuhr = set_time(month, i, j, adjust_time);
					break;
				case 3:
					newData.beginning.asr = set_time(month, i, j, adjust_time);
					break;
				case 4:
					newData.beginning.maghrib = set_time(month, i, j, adjust_time);
					break;
				case 5:
					newData.beginning.isha = set_time(month, i, j, adjust_time);
					break;
				case 6:
					newData.eqama.fajr = set_time(month, i, j, adjust_time);
					break;
				case 7:
					if (k==5) {
						newData.eqama.dhuhr = getJummaTime(month, i);
					} else {
						newData.eqama.dhuhr = set_time(month, i, j, adjust_time);
					}
					break;
				case 8:
					newData.eqama.asr = set_time(month, i, j, adjust_time);
					break;
				case 9:
					newData.eqama.maghrib = set_time(month, i, j, adjust_time);
					break;
				case 10:
					newData.eqama.isha = set_time(month, i, j, adjust_time);
					break;
			}
		}
		/* 2nd for loop ends */
		...
		/* if this_year is not leap year, feb will have 28 days instead of 29 */
	...
	// document.getElementById("month_name").innerHTML = month_name[month] + " " + " " + (this_year); /* name of the month  and the year */
...
// console.log(getJummaTime(9, 4));
var my_year = 2017;
var my_month = 10; // One lower than actual month number

j_data = [];

// console.log(ptimetable(my_year, my_month));
for (var i_year = 2015; i_year < 2020; i_year++) {
	for (var i_month = 0; i_month < 12; i_month++) {
		ptimetable(i_year, i_month);
	}
}

console.log(j_data.length);

var exportData = {"results": j_data};

fs.writeFile("prayerTimes.json", JSON.stringify(exportData, null, 4), "utf8");
